...
struct _packed_ binlog_msg_header {
    uint16_t msg_size;
    uint8_t msg_type;
};
...
bool BinLog::_start_timeout()
{
    ...
    mavlink_msg_remote_log_block_status_pack(_system_id, MAV_COMP_ID_ALL, &msg, _target_system_id, MAV_COMP_ID_ALL, MAV_REMOTE_LOG_DATA_BLOCK_START, 1);
...
int BinLog::write_msg(const struct buffer *buffer)
{
    ...
    if (trimmed_zeros) {
        ...
        mavlink_remote_log_data_block_t *binlog_data = (mavlink_remote_log_data_block_t *)payload;
        _logging_data_process(binlog_data);
    }
...
void BinLog::_send_ack(uint32_t seqno)
{
    ...
    if (seqno < _last_acked_seqno) {
        mavlink_msg_remote_log_block_status_pack(_system_id, MAV_COMP_ID_ALL, &msg, _target_system_id, MAV_COMP_ID_ALL, seqno, 1);
        _send_msg(&msg, _target_system_id);
        return;
    }
    ...
    for (uint32_t i = _last_acked_seqno; i < seqno; i++) {
        mavlink_msg_remote_log_block_status_pack(_system_id, MAV_COMP_ID_ALL, &msg, _target_system_id, MAV_COMP_ID_ALL, seqno, 0);
        _send_msg(&msg, _target_system_id);
    }
